...
else
{
	...
	switch($mode)
	{
		...
		case 'search_email':
			...
			$text = sprintf($lang['Search_for_email'], strip_tags(htmlspecialchars($email)));
		...
		case 'search_ip':
			...
			$text = sprintf($lang['Search_for_ip'], strip_tags(htmlspecialchars($ip_address)));
			...
			foreach($users as $address)
			{
				...
				if( preg_match('/(ff){1,3}$/i', $address) )
				{
					...
					$ip_sql_like .= ( $ip_sql_like != '' ) ? ' OR user_ip LIKE "' . decode_ip($address) . '%"' : 'user_ip LIKE "' . decode_ip($address) . '%"';
					$ip_like_sql .= ( $ip_like_sql != '' ) ? " OR poster_ip LIKE '".$ip_start."%'" : "poster_ip LIKE '".$ip_start."%'";
				}
				else
				{
					$ip_sql_in .= ( $ip_sql_in == '' ) ? '"' . decode_ip($address) . '"' : ', "' . decode_ip($address) . '"';
			...
			$where_sql = '';
			$where_sql .= ( $ip_sql_in != '' ) ? 'user_ip IN (' . $ip_sql_in . ')' : '';
			$where_sql .= ( $ip_sql_like != '' ) ? ( $where_sql != '' ) ? ' OR ' . $ip_sql_like : $ip_sql_like : '';
		...
		case 'search_joindate':
			...
			$text = sprintf($lang['Search_for_date'], strip_tags(htmlspecialchars($date_type)), $date_year, $date_month, $date_day);
		...
		case 'search_language':
			...
			$language_type = trim(strtolower($language_type));
		...
		case 'search_timezone':
			...
			$text = sprintf($lang['Search_for_timezone'], strip_tags(htmlspecialchars($timezone_type)));
