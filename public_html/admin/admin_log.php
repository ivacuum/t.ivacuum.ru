...
$users = array('All users' => $all_users) + array_flip($mod['moderators']) + array_flip($mod['admins']);
...
if ($log_rowset)
{
	...
	foreach ($log_rowset as $row_num => $row)
	{
		...
		$template->assign_block_vars('log', array(
			...
			'TOPIC_HREF'       => (!$topic_deleted) ? TOPIC_URL . $row['log_topic_id'] : '',
			'TOPIC_HREF_S'     => url_arg($url, $topic_key, $row['log_topic_id']),
			'TOPIC_TITLE'      => $topic_title,

			'TOPIC_ID_NEW'     => $row['log_topic_id_new'],
			'TOPIC_HREF_NEW'   => TOPIC_URL . $row['log_topic_id_new'],
...
// Selects
//
$log_type_select = array('All actions' => $all_types) + $log_action->log_type_select;
