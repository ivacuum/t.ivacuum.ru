<!-- IF AUTH_MOD -->
	<!-- IF SESSION_ADMIN -->
		...
		<form id="mod-action" action="modcp.php" method="post">
			...
			<input id="mod-submit" type="submit" value="{L_DO_SUBMIT}" class="btn" style="width: 110px;" disabled="disabled" />&nbsp;
		</form>
		</div>
...
<table width="100%">
	<tr>
		<td valign="bottom">
			...
			<!-- IF MODERATORS -->
			<p class="med">{L_MODERATORS}: <b>{MODERATORS}</b></p>
			<!-- ENDIF -->
			<!-- IF SHOW_ONLINE_LIST -->
...
<table width="100%">
	<tr>
		...
		<td class="nav" width="100%">
			...
			<em>&raquo;</em>&nbsp;<a href="{U_VIEW_FORUM_CAT}">{FORUM_CAT_TITLE}</a>
	...
	<td class="small tCenter" nowrap="nowrap" style="padding: 4px 8px;">
		<!-- BEGIN last -->
			<!-- IF f.last.FORUM_LAST_POST -->
				...
				<p class="genmed" style="margin-top:4px;">
				...
				<p class="genmed">{f.last.LAST_POST_TIME}</p>
				<p class="genmed" style="margin-top:3px;">
					<!-- IF f.last.LAST_POST_USER_ID -->
					<a href="{PROFILE_URL}{f.last.LAST_POST_USER_ID}">{f.last.LAST_POST_USER_NAME}</a>
					<!-- ELSE -->
					{f.last.LAST_POST_USER_NAME}
...
<tr>
	...
	<tr>
		<!-- IF AUTH_MOD -->
		<td class="genmed bold nowrap" style="padding: 0px 0px 0px 4px;">
			<span id="show_mod_options"><a href="#" class="genmed bold">{L_MODERATE_FORUM}</a></span>
		</td>
		<td class="med" style="padding: 0px 4px 2px 4px;">|</td>
		<td class="genmed nowrap" style="padding: 0px 0px 0px 0px;">{L_TOPICS_PER_PAGE}:</td>
		<td class="genmed nowrap" style="padding: 0px 0px 0px 3px;">
			<form id="tpp" action="{PAGE_URL_TPP}" method="post">{SELECT_TPP}</form>
		</td>
		<!-- ENDIF / AUTH_MOD -->

		<td class="genmed bold nowrap tRight w100">
			&nbsp;
			<!-- IF LOGGED_IN -->
...
<tr id="tr-{t.TOPIC_ID}">
	...
	<div class="torTopic">
		<!-- BEGIN tor -->
		<!-- IF t.TOR_STATUS == 0 --><b><span style="color: purple;">*</span></b><!-- ENDIF -->
		<!-- IF t.TOR_STATUS == 1 --><b><span style="color: red;">x</span></b><!-- ENDIF -->
		<!-- IF t.TOR_STATUS == 2 --><b><span style="color: green;">&radic;</span></b><!-- ENDIF -->
		<!-- IF t.TOR_STATUS == 3 --><b><span style="color: blue;">D</span></b><!-- ENDIF -->
		<!-- IF t.TOR_STATUS == 4 --><b><span style="color: red;">!</span></b><!-- ENDIF -->
		<!-- IF t.TOR_STATUS == 5 --><b><span style="color: red;">?</span></b><!-- ENDIF -->
		<!-- IF t.TOR_STATUS == 6 --><b><span style="color: green;">#</span></b><!-- ENDIF -->
		<!-- IF t.TOR_STATUS == 7 --><b><span style="color: red;">&copy;</span></b><!-- ENDIF -->&nbsp;
		<!-- END tor -->
	...
	<!-- BEGIN tor -->
		<div title="{L_DL_TORRENT}">
			...
			<div style="padding-top: 2px" class="small"><!-- IF t.TOR_FROZEN -->{t.tor.TOR_SIZE}<!-- ELSE --><a href="{DOWNLOAD_URL}{t.tor.ATTACH_ID}" class="genmed" style="text-decoration: none;">{t.tor.TOR_SIZE}</a><!-- ENDIF --></div>
		</div>
	<!-- END tor -->
	</td>

	<td class="tCenter genmed nowrap" style="padding: 3px 4px 2px;">
	<p>
	...
	<td class="tCenter genmed nowrap" style="padding: 3px 6px 2px;">
...
<tr id="tr-{t.TOPIC_ID}">
	...
	<td class="tCenter nowrap genmed" style="padding: 1px 6px 2px;">
		<p>{t.LAST_POST_TIME}</p>
		<p>
			<!-- IF t.LAST_POSTER_HREF --><a href="{PROFILE_URL}{t.LAST_POSTER_HREF}">{t.LAST_POSTER_NAME}</a><!-- ELSE -->{t.LAST_POSTER_NAME}<!-- ENDIF -->
			<a href="{POST_URL}{t.LAST_POST_ID}#{t.LAST_POST_ID}">{ICON_LATEST_REPLY}</a>
