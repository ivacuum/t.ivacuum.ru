...
<tr>
	...
	{S_HIDDEN_FIELDS}
		...
		<div style="float: right;">
			<input type="submit" value="{L_VIEW_INFORMATION}" class="btn" />
		</div>
	</form>
	</td>
</tr>
...
<tr>
	...
	<p class="mrg_4">
		<input class="btn btn-primary" type="submit" name="joingroup" value="{L_JOIN_A_GROUP}" onclick="return confirm('{L_JOIN_A_GROUP}?');" />&nbsp;&nbsp;
	</p>
	<!-- ENDIF -->
	<!-- IF SHOW_UNSUBSCRIBE_CONTROLS -->
	<p class="mrg_4">
		<input class="btn btn-primary" type="submit" name="{CONTROL_NAME}" value="{L_UNSUBSCRIBE_GROUP}" onclick="return confirm('{L_UNSUBSCRIBE_GROUP}?');" />&nbsp;&nbsp;
	</p>
	<!-- ENDIF -->
	</td>
</tr>
...
<tr>
	...
	<td>
		...
		<p class="mrg_4">
			<input class="btn btn-primary" type="submit" name="groupstatus" value="{L_UPDATE}" />
		</p>
	</td>
</tr>
...
<tr>
	<td colspan="7" class="cat" style="padding: 2px 12px;">
		<p id="add_group_member" class="floatL">
			...
			<input type="submit" name="add" value="{L_ADD_MEMBER}" class="btn btn-primary" />
			<input type="button" name="usersubmit" value="{L_FIND_USERNAME}" class="btn" onclick="window.open('{U_SEARCH_USER}', '_phpbbsearch', 'HEIGHT=250,resizable=yes,WIDTH=400');return false;" />
		</p>
		<p class="floatR" style="padding-top: 1px;">
			<input type="submit" name="remove" value="{L_REMOVE_SELECTED}" class="btn btn-primary" onclick="return confirm('{L_REMOVE_SELECTED}?');" />
		</p>
	</td>
</tr>
<!-- END switch_mod_option -->
...
<tr>
	<td class="cat" colspan="7">
		<input type="submit" name="approve" value="{L_APPROVE_SELECTED}" onclick="return confirm('{L_APPROVE_SELECTED}?');" class="btn btn-primary" />
		&nbsp;
		<input type="submit" name="deny" value="{L_DENY_SELECTED}" onclick="return confirm('{L_DENY_SELECTED}?');" class="btn" />
	</td>
</tr>
</table>
