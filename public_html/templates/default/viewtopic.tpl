...
<!-- IF HIDE_UPLOAD -->.upload { display: none; }<!-- ENDIF -->
<!-- IF HIDE_DOWNLOAD -->.download { display: none; }<!-- ENDIF -->
...
<!-- IF SPOILER_OPENED -->.spoiler-body { display: block; }<!-- ENDIF -->
</style>

<script type="text/javascript">
$(document).ready(function() {
<!-- IF SPOILER_OPENED -->
  $('div.spoiler-head').addClass('unfolded');
<!-- ENDIF -->
});
</script>
<!-- ENDIF / LOGGED_IN -->
...
<tr>
	...
	<tr>
		...
		<td class="small bold nowrap" style="padding: 0px 0px 0px 4px;">
			<!-- IF IN_MODERATION -->{L_MODERATE_TOPIC}<!-- ELSE --><a href="{PAGE_URL}&amp;mod=1&amp;start={PAGE_START}" class="genmed bold">{L_MODERATE_TOPIC}</a><!-- ENDIF -->
		</td>
		<!-- IF SELECT_PPP -->
		<td class="med" style="padding: 0px 4px 2px 4px;">|</td>
		<td class="genmed nowrap" style="padding: 0px 0px 0px 0px;">{L_SELECT_PPP}</td>
		<td class="genmed nowrap" style="padding: 0px 0px 0px 3px;">
			<form id="ppp" action="{PAGE_URL_PPP}" method="post">{SELECT_PPP}</form>
		</td>
		<!-- ENDIF / SELECT_PPP -->
		<!-- ENDIF / AUTH_MOD -->

		<td class="genmed bold nowrap tRight" width="100%">
			&nbsp;
			<!-- IF LOGGED_IN -->
		...
		<td class="nowrap" style="padding: 0px 4px 2px 4px;">
			<form action="search.php?t={TOPIC_ID}&amp;dm=1&amp;s=1" method="post" onsubmit="var txt=$('#search-text').val(); return !(txt=='{L_SEARCH_IN_TOPIC}' || !txt);">
				...
				<input class="btn btn-small btn-primary" type="submit" value="&raquo;" style="width: 30px;" />
			</form>
		</td>
	</tr>
	</table>
...
<tr>
	...
	<!-- ELSE -->
		...
		<p class="torrents"><img src="{STATIC_PATH}/i/tracker/icon_up.gif" alt="" style="vertical-align: text-top;"> {postrow.SEEDING} &nbsp;<img src="{STATIC_PATH}/i/tracker/icon_down.gif" alt="" style="vertical-align: text-top;"> {postrow.LEECHING}</p>
	...
	<td class="message td2" rowspan="2">
		...
		<div class="post_head">
			<p style="float: left; padding: 8px 0;">
				<!-- IF postrow.IS_UNREAD -->{MINIPOST_IMG_NEW}<!-- ELSE -->{MINIPOST_IMG}<!-- ENDIF -->
				<a class="genmed" href="{POST_URL}{postrow.POST_ID}#{postrow.POST_ID}" title="{L_POST_LINK}">{postrow.POST_DATE}</a>
...
<tr>
	...
	<!-- IF postrow.POSTER_BOT || not postrow.PROFILE -->
		&nbsp;
	<!-- ELSE -->
		<div style="padding: 3px 0;" class="post_btn_2">
			<!--<a class="btn btn-left btn-icons" href="{PROFILE_URL}{postrow.POSTER_ID}"><img src="{STATIC_PATH}/i/_/card_address.png" alt="" />Инфо</a><a class="btn btn-right btn-icons" href="{PM_URL}{postrow.POSTER_ID}"><img src="{STATIC_PATH}/i/_/mail_send.png" alt="" />ЛС</a>-->
			<a class="txtb" href="{PROFILE_URL}{postrow.POSTER_ID}">[Профиль]</a> &nbsp;<a class="txtb" href="{PM_URL}{postrow.POSTER_ID}">[ЛС]</a>
		</div>
	<!-- ENDIF -->
	...
	<input type="hidden" name="mode" value="split" />
		...
		<tr>
			<td colspan="2" align="center" class="row3">
				<input class="btn btn-negative" type="submit" name="delete_posts" id="del" value="{L_DELETE_POSTS}" disabled="disabled" onclick="return window.confirm('{L_DELETE_POSTS}?');" />
				<input class="btn" type="submit" name="split_type_all" id="spl_all" value="{L_SPLIT_POSTS}" disabled="disabled" onclick="return window.confirm('{L_SPLIT_POSTS}?');" />
				<input class="btn" type="submit" name="split_type_beyond" id="spl_b" value="{L_SPLIT_AFTER}" disabled="disabled" onclick="return window.confirm('{L_SPLIT_AFTER}?');" />
				<label for="spl_cnf">
					{L_CONFIRM}
...
<tr>
	<th class="thHead gen"><b>Быстрый ответ</b></th>
</tr>
<tr>
...
<tr>
	...
	<form method="post" action="{S_POST_DAYS_ACTION}">
		...
		{S_SELECT_POST_ORDER}&nbsp;
		<input type="submit" value="{L_GO}" class="btn btn-small" name="submit" />
	</form>
	</td>
</tr>
</table>
