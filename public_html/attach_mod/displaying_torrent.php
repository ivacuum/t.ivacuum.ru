...
$ratio_warning_url_help = $bb_cfg['bt_ratio_warning_url_help'];
...
if ($tor_reged && $tor_info)
{
	...
	if (($min_ratio_dl || $min_ratio_warn) && $user_status != DL_STATUS_COMPLETE && $bt_user_id != $poster_id)
	{
	...
	else
	{
		...
		$template->assign_block_vars('postrow.attach.tor_reged', array(
			...
			'DOWNLOAD_COUNT' => sprintf($lang['Download_number'], $download_count),
			'LAST_DL_TIME'   => ( $tor_info['last_dl_time'] ) ? delta_time($tor_info['last_dl_time']) . ' назад' : 'никогда',
			'THANKS'         => '<span id="thanks' . $attach_id . '">' . $thanks . '</span>',
	...
	if ($bb_cfg['show_tor_info_in_dl_list'])
	{
		$template->assign_vars(array(
			...
			'TOR_LAST_DL_TIME' => ( $tor_info['last_dl_time'] ) ? delta_time($tor_info['last_dl_time']) . ' назад' : 'никогда',
		));
	}
	...
	if ($show_peers)
	{
		...
		if( $s_mode == 'count' )
		{
			$leechers = $leech_count = $tor_info['leechers'];
			$seeders = $seed_count = $tor_info['seeders'];

			$template->assign_vars(array(
				'LEECH_COUNT'    => $tor_info['leechers'],
				'SEED_COUNT'     => $tor_info['seeders'],
				'TOR_SPEED_UP'   => ( $tor_info['speed_up'] ) ? humn_size($tor_info['speed_up'], 0, 'KБ') . '/сек' : 0,
				'TOR_SPEED_DOWN' => ( $tor_info['speed_down'] ) ? humn_size($tor_info['speed_down'], 0, 'KБ') . '/сек' : 0)
			);
		}
		else
		if ($peers = $db->fetch_rowset($sql))
		{
			...
			if ($s_mode == 'count')
			{
				...
				$template->assign_vars(array(
					'TOR_SPEED_UP'   => ($tor_speed_up)   ? humn_size($tor_speed_up, 0, 'KБ') .'/сек' : '0 KБ/сек',
					'TOR_SPEED_UP_INT'	=> ( $tor_speed_up ) ? $tor_speed_up : 0,
					'TOR_SPEED_DOWN' => ($tor_speed_down) ? humn_size($tor_speed_down, 0, 'KБ') .'/сек' : '0 KБ/сек',
					'TOR_SPEED_DOWN_INT'	=> ( $tor_speed_down ) ? $tor_speed_down : 0)
				);
			}
			...
			foreach ($peers as $pid => $peer)
			{
				...
				if ($s_mode == 'full')
				{
					$ip    = /*bt_show_ip($peer['ip'])*/(IS_MOD || IS_ADMIN) ? $peer['ip'] : '';
					...
					if ($peer['seeder'])
					{
						...
						$compl_perc = ($tor_size) ? str_replace(',', '.', round(($p_max_up / $tor_size), 1)) : 0;
					}
					else
					{
						$x = 'l';
						...
						if (!defined('LEECHER_EXIST'))
						{
							...
							$template->assign_block_vars("$x_full", array(
								...
								'LEECHERS_UP_TOT'   => humn_size($sp_up_tot[$x], 0, 'KB') .'/сек',
								'LEECHERS_DOWN_TOT' => humn_size($sp_down_tot[$x], 0, 'KB') .'/сек'
							));
					...
					$sp_up    = ($peer['speed_up'])   ? humn_size($peer['speed_up'],   0, 'KB') .'/с' : '-';
					$sp_down  = ($peer['speed_down']) ? humn_size($peer['speed_down'], 0, 'KB') .'/с' : '-';
					...
					$template->assign_block_vars("$x_full.$x_row", array(
						'INT_DOWN_TOTAL'	=> $p_max_down,
						'INT_UP_TOTAL'		=> $p_max_up,
						'INT_SPEED_DOWN'	=> $peer['speed_down'],
						'INT_SPEED_UP'		=> $peer['speed_up'],
						'ROW_BGR'      => $row_bgr,
