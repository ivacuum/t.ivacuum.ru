...
function prepare_bbcode_template($bbcode_tpl)
{
	...
	$bbcode_tpl['name'] = str_replace('{NAME}', '\\1', $bbcode_tpl['name']);
	...
	$bbcode_tpl['url5'] = str_replace('{URL}', '#\\1', $bbcode_tpl['url_name']);
...
function bbencode_second_pass($text, $uid)
{
	...
	// [name=xxx] code..
	$patterns[] = "#\[name=([\d\.]+)\]#is";
	$replacements[] = $bbcode_tpl['name'];

	// matches a [url]xxxx://www.phpbb.com[/url] code..
	$patterns[] = "#\[url\]([\w]+?://([\w\#$%&~/.\x{7f}-\x{ff}\-;:=,?@\]+]|\[(?!url=))*?)\[/url\]#is";
	...
	// [url=#xxx]Link to the part of post[/url]
	$patterns[] = "#\[url=\#([\d\.]+)\]([^?\n\r\t].*?)\[/url\]#is";
