...
class Template {
	...
	function set_rootdir($dir)
	{
		...
		$this->tpldir = SITE_DIR . 'templates/';
		$this->root = $dir;
	...
	{
		$filename = clean_filename(str_replace(SITE_DIR . 'templates/', '', $filename));
...
#		$code = preg_replace('#\{(L_([a-z0-9\-_]+?))\}#i', '<'.'?php if (isset($L[\'$2\'])) { echo $L[\'$2\']; } else { echo ($V[\'$1\']) ? $V[\'$1\'] : $this->lang_error(\'$2\'); } ?'.'>', $code);
		$code = preg_replace('#\{(L_([a-z0-9\-_]+?))\}#i', '<?php echo isset($L[\'$2\']) ? $L[\'$2\'] : $V[\'$1\']; ?>', $code);
		$code = preg_replace('#\{(\$[a-z_][a-z0-9_\->\'\"\[\]]*?)\}#i', '<?php echo $1; ?>', $code);
		$code = preg_replace('#\{(\#([a-z_][a-z0-9_]*?))\}#i', '<?php echo $2; ?>', $code);
	...
	function compile_tag_if($tag_args, $elseif)
	{
		...
		for ($i = 0; $i < $tokens_cnt; $i++)
		{
			...
			switch ($token)
			{
				...
				default:
					if (preg_match('#^(([a-z0-9\-_]+?\.)+?)?(\$)?([a-z_][a-z0-9_\->\'\"\[\]]+)$#i', $token, $varrefs))
					{
						if (!empty($varrefs[1]))
						{
							$token = $this->generate_block_data_ref(substr($varrefs[1], 0, -1), true) . '[\'' . $varrefs[4] . '\']';
						}
						else if ($varrefs[3])
						{
							$token = $varrefs[3] . $varrefs[4];
						}
						else if ($tokens_cnt == 1)
						{
							$token = '!empty($V[\''. $varrefs[4] .'\'])';
						}
						else
						{
							$token = '$V[\''. $varrefs[4] .'\']';
