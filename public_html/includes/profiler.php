...
class profiler extends console
{
	...
	*/
	private function display_profiler()
	{
		...
		foreach( $this->output['logs'] as $log )
		{
			$template->assign_block_vars('logs', array_change_key_case($log, CASE_UPPER));
		}

		foreach( $this->output['files'] as $file )
		{
			$template->assign_block_vars('files', array_change_key_case($file, CASE_UPPER));
		}

		foreach( $this->output['queries'] as $query )
		{
			$template->assign_block_vars('queries', array_change_key_case($query, CASE_UPPER));
		}
		...
		$template->set_filename('profiler', 'profiler.html');
		$template->pparse('profiler');
	}
}
