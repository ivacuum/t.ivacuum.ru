...
class sql_db
{
    ...
    function connect ()
    {
        ...
        $connect_type = ($this->pconnect) ? 'mysqli_pconnect' : 'mysqli_connect';
    ...
    function sql_query ($query, $type = DEFAULT_QUERY_TYPE)
    {
        ...
        $query_function = ($type === 'unbuffered') ? 'mysqli_unbuffered_query' : 'mysqli_query';
        $start_time = microtime(true);

        if (!$this->result = $query_function($this->link, $query))
        {
            $this->log_error();
        }
    ...
    function sql_numrows ($result = false)
    {
        ...
        {
            $num_rows = is_object($result) ? mysqli_num_rows($result) : false;
        }
    ...
    {
        return is_object($this->link) ? mysqli_affected_rows($this->link) : -1;
    }

    /**
     * Fetch current row
     */
    function sql_fetchrow ($result, $result_type = MYSQLI_ASSOC)
    {
        return is_object($result) ? mysqli_fetch_array($result, $result_type) : false;
    }

    /**
     * Alias of sql_fetchrow()
     */
    function fetch_next ($result, $result_type = MYSQLI_ASSOC)
    {
     ...
     * Fetch all rows
     */
    function sql_fetchrowset ($result, $result_type = MYSQLI_ASSOC)
    {
        $rowset = array();
    ...
    function sql_freeresult ($result = false)
    {
        if ($result OR $result = $this->result)
        {
            $return_value = is_object($result) ? mysqli_free_result($result) : false;
        }
    ...
    {
        return is_object($result) ? mysqli_num_fields($result) : false;
    }
...
#			return true;
    ...
    function query_info ()
    {
        ...
        if (is_object($this->link) AND $ext = mysqli_info($this->link))
        {
            $info[] = "$ext";
        }
