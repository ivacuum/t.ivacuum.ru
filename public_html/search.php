...
else
{
	...
	$SQL['SELECT'][] = "
		...
		tor.seeders, tor.leechers, tor.speed_down + tor.speed_up AS speed,
	...
	foreach( $items_display as $row_num => $item_id )
	{
		...
		$template->assign_block_vars('t', array(
			...
			'LEECHERS'      => $topic['leechers'],
			'SEEDERS'       => $topic['seeders'],
			'SPEED'         => ( $topic['speed'] > 0 ) ? humn_size($topic['speed']) . '/с' : '-',
...
### TEMP ###
#		preg_match("#MATCH \((\w).*?\) AGAINST \('(.*?)' IN BOOLEAN MODE\)#", $SQL['WHERE'][count($SQL['WHERE'])-1], $m);
...
function username_search($search_match)
{
	...
	if( !empty($search_match) )
	{
		...
		foreach( $db->fetch_rowset($sql) as $row )
		{
			$username = htmlCHR(html_entity_decode($row['username']));
