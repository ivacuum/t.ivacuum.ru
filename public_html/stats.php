...
switch( $time )
{
	...
	case 'all': $date_format = '%d.%m'; $points_count = 34560; $threshold = 10; break;
...
switch( $mode )
{
	case 'peers':
		...
		$select_what = 'trend_peers, trend_seeders, trend_leechers, trend_users_online, trend_visitors, FROM_UNIXTIME(trend_time) AS date';
...
for( $i = 0, $len = sizeof($ary); $i < $len; $i++ )
{
	...
	foreach( $ary[$i] as $key => $value )
	{
		...
		${$key} = sprintf('%s["%s",%s],', ${$key}, $ary[$i]['date'], $value);
...
$template->assign_vars(array(
	...
	'TREND_PEERS'            => substr($trend_peers, 0, -1),
	'TREND_POSTS'            => substr($trend_posts, 0, -1),
